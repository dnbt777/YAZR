...
pub const Camera = struct {
    ...
    // takes an NxM of rays and gets all of their colors...?
    pub fn ray_color_cuda(ray: rt.Ray, depth: u32, world: hittable.Hittable) @Vector(3, f32) {
        if (depth <= 0) {
            return utils.splat(0);
        }
    ...
    pub fn render_cuda(self: Camera, world: hittable.Hittable) !void {
        var file = try fs.cwd().createFile(
            "test.ppm",
            .{ .read = true },
        );
        ...
        // pixel_color = 0 matrix
        // for _ in samples_per_pixel:
        //     pixel_color += get_ray_color(i, j, max_depth, world) // question is.. how to handle world????????
        ...
        // then for pixel in resulting matrix, write it to
    }
